...
/**
 * Manually decode an ScVal to a JS primitive, safely handling all types
 * including booleans (which scValToNative can mishandle in some SDK versions).
 */
function decodeScVal(val) {
  ...
  switch (t) {
    ...
    default:
      // Fallback: try scValToNative for anything else
      try {
...
export async function fetchPolls() {
  ...
  // Use our safe decoder instead of scValToNative which fails on bool fields
  const outerMap = decodeScVal(retval);
  if (!outerMap || typeof outerMap !== "object") return [];

  const polls = Object.entries(outerMap).map(([id, poll]) => ({
    id: Number(id),
